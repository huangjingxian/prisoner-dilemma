...
def a():
	...
	# print oppo_score
	top5=[]
	users[number]=[room,AI_player,rm_mate,rd_arr,oppo_score,score] # room, opponent type, how many roommate, the array of idx of all, opponent scoreboard
...
def complex():
	...
	# 	# print operation
		...
		if (rd_arr[0] ==top5[0][0]) or (rd_arr[0] ==top5[1][0]) or (rd_arr[0] ==top5[2][0]) or (rd_arr[0] ==top5[3][0]) or (rd_arr[0] ==top5[4][0]):
			flag = 1 
		db.execute('insert into scores (idx,oppo_type,room_num,name1,oper1,delta1,tot1,name2,oper2,delta2,tot2) values (?,?,?,?,?,?,?,?,?,?,?)',[number,AI_type,room,rd_arr[0],player_action,score_add,score,oppo,operation,delta_AI,oppo_score[oppo]])
		db.commit()
		db.execute('insert into scores (idx,oppo_type,room_num,name1,oper1,delta1,tot1,name2,oper2,delta2,tot2) values (?,?,?,?,?,?,?,?,?,?,?)',[number,AI_type,room,oppo,operation,delta_AI,oppo_score[oppo],rd_arr[0],player_action,score_add,score])
		db.commit()
